...
function renderComponentList() {
  ...
  if (!settings.components.length) { list.html('<div class="st-esg-empty">还没有组件。可以手动添加，也可以点“刷新可选来源”，从预设、世界书或角色卡条目里勾选导入。</div>'); return; }
  list.html(settings.components.map((item, index) => `<div class="st-esg-component-item" data-index="${index}"><label class="st-esg-checkbox"><input class="st-esg-component-enabled" type="checkbox" ${item.enabled === false ? '' : 'checked'} /><span>${escapeHtml(item.name || '未命名组件')} · ${escapeHtml(item.scope || '全局')}</span></label><button class="menu_button st-esg-component-delete" type="button">删除</button><pre>${escapeHtml(item.content || '')}</pre></div>`).join(''));
  ...
  $t('.st-esg-component-delete').on('click', function () { settings.components.splice(Number($(this).closest('.st-esg-component-item').data('index')), 1); saveSettings(); renderComponentList(); });
}

function addComponent() {
  const name = textOf($t('#st-esg-component-name').val());
  const scope = textOf($t('#st-esg-component-scope').val()) || '全局';
...
function addCandidate(candidates, group, source, scope, name, content, meta = '') {
  const clean = textOf(content);
  if (!clean) return;
  const key = `${group}::${source}::${scope}::${name}::${clean.slice(0, 200)}`;
  if (!candidates.some((item) => item.key === key)) candidates.push({ key, group, source, scope, name: name || '未命名条目', content: clean, meta });
}
...
async function scanImportCandidates() {
  ...
  addCandidate(candidates, '角色卡', currentChar?.name || '当前角色', '角色', '角色描述', charData?.description || currentChar?.description);
  addCandidate(candidates, '角色卡', currentChar?.name || '当前角色', '角色', '性格', charData?.personality || currentChar?.personality);
  addCandidate(candidates, '角色卡', currentChar?.name || '当前角色', '角色', '场景', charData?.scenario || currentChar?.scenario);
  addCandidate(candidates, '角色卡', currentChar?.name || '当前角色', '角色', '开场白', charData?.first_mes || currentChar?.first_mes);
  addCandidate(candidates, '角色卡', currentChar?.name || '当前角色', '角色', '示例对话', charData?.mes_example || currentChar?.mes_example);
  addCandidate(candidates, '角色卡', currentChar?.name || '当前角色', '角色', '角色系统提示词', charData?.system_prompt);
  addCandidate(candidates, '角色卡', currentChar?.name || '当前角色', '角色', '角色后置提示词', charData?.post_history_instructions);
  for (const [key, prompt] of Object.entries(context.extensionPrompts || {})) {
    if (prompt?.value) addCandidate(candidates, '当前注入提示', '已启用注入', '预设', key, prompt.value, [prompt?.role ? `role: ${prompt.role}` : '', Number.isFinite(prompt?.position) ? `position: ${prompt.position}` : '', Number.isFinite(prompt?.depth) ? `depth: ${prompt.depth}` : ''].filter(Boolean).join(' · '));
  }
  for (const [managerId, label] of [['sysprompt', '系统提示词'], ['context', '上下文模板'], ['instruct', '指令模板'], ['reasoning', '思考/Reasoning模板']]) {
    try {
      ...
      if (data) for (const [field, value] of Object.entries(data)) if (typeof value === 'string' && value.trim()) addCandidate(candidates, `当前预设：${label}`, presetName, '预设', field, value);
    } catch (error) { console.warn(`[${EXTENSION_ID}] 读取预设失败`, managerId, error); }
  }
  addCandidate(candidates, '用户设定', '当前用户', '全局', '用户人格', context.powerUserSettings?.persona_description);
  ...
  const namesToLoad = [...new Set([...(Array.isArray(selectedWorldNames) ? selectedWorldNames : [selectedWorldNames]).filter(Boolean), ...worldNames])];
  if (typeof context.loadWorldInfo === 'function') {
    for (const worldName of namesToLoad) {
      try {
        const world = await context.loadWorldInfo(worldName);
        for (const entry of Object.values(world?.entries || {}).sort((a, b) => Number(a?.order ?? 0) - Number(b?.order ?? 0))) {
          if (!entry?.content) continue;
          const title = entry.comment || entry.key?.join?.(', ') || `世界书条目 ${entry.uid ?? ''}`;
          const meta = [entry.disable ? '禁用' : '启用', entry.constant ? '常驻' : '', Number.isFinite(entry.order) ? `顺序 ${entry.order}` : '', Number.isFinite(entry.depth) ? `深度 ${entry.depth}` : ''].filter(Boolean).join(' · ');
          addCandidate(candidates, `世界书：${worldName}`, worldName, '全局', title, entry.content, meta);
        }
      } catch (error) { console.warn(`[${EXTENSION_ID}] 扫描世界书失败`, worldName, error); }
    }
  }
...
function renderImportCandidates() {
  ...
  if (!importCandidates.length) { box.html('<div class="st-esg-empty">还没有可选来源。点击“刷新可选来源”后，会按预设、世界书、角色卡分组列出条目。</div>'); return; }
  ...
  box.html([...groups.entries()].map(([groupName, items]) => `<section class="st-esg-import-group"><div class="st-esg-import-group-head"><div><div class="st-esg-import-group-title">${escapeHtml(groupName)}</div><div class="st-esg-card-desc">${items.length} 个可选条目</div></div><button class="menu_button st-esg-import-group-toggle" type="button">本组全选</button></div><div class="st-esg-import-group-list">${items.map((item) => `<div class="st-esg-import-item" data-index="${item.index}"><label class="st-esg-checkbox"><input class="st-esg-import-check" type="checkbox" /><span>${escapeHtml(item.name)} · ${escapeHtml(item.scope)} · ${escapeHtml(item.source)}</span></label>${item.meta ? `<div class="st-esg-import-meta">${escapeHtml(item.meta)}</div>` : ''}<pre>${escapeHtml(item.content.slice(0, 1200))}</pre></div>`).join('')}</div></section>`).join(''));
...
function importCheckedCandidates() {
  ...
  for (const checkbox of checked) {
    ...
    if (item) settings.components.push({ id: String(Date.now() + Math.random()), name: item.name, scope: item.scope, content: item.content, enabled: true, source: item.source });
  }
  ...
  dialog.innerHTML = `
    <div class="st-esg-shell">
      ...
      <div class="st-esg-panel-body">
        <section class="st-esg-hero"><div><div class="st-esg-hero-title">把文末组件从正文注意力里拆出来</div><div class="st-esg-hero-desc">正文先专心讲故事，状态栏由插件单独生成、预览、注入。</div></div><div class="st-esg-status-pill"><span class="st-esg-dot"></span><span id="st-esg-status">插件已加载。</span></div></section>
        ...
        <section class="st-esg-tab-panel" data-tab-panel="components"><div class="st-esg-card"><div class="st-esg-card-head"><div><div class="st-esg-card-title">添加组件</div><div class="st-esg-card-desc">可以手动添加，也可以从当前预设、世界书条目、角色卡字段里勾选导入。</div></div></div><div class="st-esg-grid"><label>组件名<input id="st-esg-component-name" class="text_pole" type="text" placeholder="例如：人物状态栏" /></label><label>分类<select id="st-esg-component-scope" class="text_pole"><option>全局</option><option>预设</option><option>角色</option></select></label></div><textarea id="st-esg-component-content" class="text_pole textarea_compact st-esg-textarea" rows="5" placeholder="在这里粘贴状态栏格式、要求或组件提示词。"></textarea><div class="st-esg-actions-row"><div id="st-esg-add-component" class="menu_button menu_button_icon st-esg-secondary-action"><i class="fa-solid fa-plus"></i><span>添加到组件库</span></div><div id="st-esg-scan-components" class="menu_button menu_button_icon st-esg-secondary-action"><i class="fa-solid fa-list-check"></i><span>刷新可选来源</span></div><div id="st-esg-import-components" class="menu_button menu_button_icon st-esg-secondary-action"><i class="fa-solid fa-file-import"></i><span>导入勾选条目</span></div></div></div><div id="st-esg-import-candidates" class="st-esg-import-list"><div class="st-esg-empty">还没有可选来源。点击“刷新可选来源”后，会按预设、世界书、角色卡分组列出条目。</div></div><div id="st-esg-component-list" class="st-esg-component-list"></div></section>
