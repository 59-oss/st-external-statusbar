...
const targetWindow = (() => {
  try {
    return window.parent?.document?.body ? window.parent : window;
  } catch (_) {
    return window;
  }
})();
...
function $t(selectorOrHtml) {
  return $(selectorOrHtml, targetDoc);
}
...
function togglePanel(forceOpen) {
  const panel = $t('#st-external-statusbar-panel');
  if (!panel.length) return;
  const shouldOpen = typeof forceOpen === 'boolean' ? forceOpen : panel.hasClass('st-esg-panel-hidden');
  if (shouldOpen) {
    targetDoc.body.appendChild(panel[0]);
    panel.css({ position: 'fixed', inset: '0', display: 'flex', zIndex: '2147483647' });
  }
...
function renderMagicWandMenuButton(retry = 0) {
  ...
  if (!menu) {
    targetWindow.setTimeout(() => renderMagicWandMenuButton(retry + 1), 500);
    return;
  }
...
function renderFloatingBall() {
  ...
  const onMove = (event) => {
    ...
    ball.css({
      left: `${clamp(originLeft + dx, 0, targetWindow.innerWidth - 46)}px`,
      bottom: `${clamp(originBottom - dy, 0, targetWindow.innerHeight - 46)}px`,
    });
  };
...
function renderComponentList() {
  ...
  $t('.st-esg-component-delete').on('click', function () {
    const index = Number($(this).closest('.st-esg-component-item').data('index'));
...
function addComponent() {
  ...
  $t('#st-esg-component-name').val('');
  $t('#st-esg-component-content').val('');
  saveSettings();
  renderComponentList();
  setStatus('已添加组件。');
}
  ...
  for (const [managerId, label] of presetSections) {
    try {
      ...
      for (const [field, value] of Object.entries(data)) {
        if (typeof value === 'string' && value.trim()) addStructuredCandidate(candidates, `当前预设：${label}`, presetName, '预设', field, value);
      }
    } catch (error) {
  ...
  if (typeof context.loadWorldInfo === 'function') {
    for (const worldName of namesToLoad) {
      try {
        ...
        const entries = Object.values(world?.entries || {}).sort((a, b) => Number(a?.order ?? 0) - Number(b?.order ?? 0));
...
function renderImportCandidates() {
  ...
  box.html([...groups.entries()].map(([groupName, items]) => `
    <section class="st-esg-import-group">
      <div class="st-esg-import-group-head">
        <div><div class="st-esg-import-group-title">${escapeHtml(groupName)}</div><div class="st-esg-card-desc">${items.length} 个可选条目</div></div>
      ...
      <div class="st-esg-import-group-list">
        ${items.map((item) => `
          <div class="st-esg-import-item" data-index="${item.index}">
            <label class="st-esg-checkbox"><input class="st-esg-import-check" type="checkbox" /><span>${escapeHtml(item.name)} · ${escapeHtml(item.scope)} · ${escapeHtml(item.source)}</span></label>
  ...
  $t('.st-esg-import-group-toggle').on('click', function () {
...
function importCheckedCandidates() {
  ...
  for (const checkbox of checked) {
    ...
    if (!item) continue;
    settings.components.push({ id: String(Date.now() + index), name: item.name, scope: item.scope, content: item.content, enabled: true, source: item.source });
  }

  saveSettings();
  renderComponentList();
  $t('.st-esg-import-check').prop('checked', false);
  setStatus(`已导入 ${checked.length} 个组件。`);
}

function renderPluginPanel() {
  if (targetDoc.getElementById('st-external-statusbar-panel')) return;
      ...
      <div class="st-esg-shell">
        ...
        <div class="st-esg-panel-body">
          ...
          <section class="st-esg-tab-panel" data-tab-panel="workspace">
            ...
            <div class="st-esg-workflow"><div class="st-esg-step"><b>1</b><span>读取最新助手回复</span></div><div class="st-esg-step"><b>2</b><span>按组件与任务生成</span></div><div class="st-esg-step"><b>3</b><span>预览后写回正文末尾</span></div></div>
          </section>

          <section class="st-esg-tab-panel" data-tab-panel="runtime">
            <div class="st-esg-card">
              <div class="st-esg-card-head"><div><div class="st-esg-card-title">运行模式</div><div class="st-esg-card-desc">控制插件是否监听正文生成，以及生成后是否自动注入。</div></div><label class="st-esg-switch"><input id="st-esg-enabled" type="checkbox" /><span></span><em>启用</em></label></div>
              <select id="st-esg-mode" class="text_pole st-esg-select"><option value="autoInject">自动生成，并自动注入最新回复</option><option value="autoReview">自动生成，但手动确认注入</option><option value="manual">手动点击生成，手动注入</option></select>
            </div>
            <div class="st-esg-card"><div class="st-esg-card-head"><div><div class="st-esg-card-title">生成任务指令</div><div class="st-esg-card-desc">告诉插件“要补什么状态栏组件”。这会作为外置生成任务的核心约束。</div></div></div><textarea id="st-esg-task" class="text_pole textarea_compact st-esg-textarea" rows="7"></textarea></div>
          </section>
          ...
          <section class="st-esg-tab-panel" data-tab-panel="api">
            <div class="st-esg-card">
              ...
              <div class="st-esg-grid"><label>API 地址<input id="st-esg-api-url" class="text_pole" type="text" placeholder="例如 https://api.openai.com/v1" /></label><label>模型名称<input id="st-esg-api-model" class="text_pole" type="text" placeholder="例如 gpt-4o-mini / deepseek-chat" /></label><label>最大输出<input id="st-esg-max-tokens" class="text_pole" type="number" min="1" step="1" /></label><label>温度<input id="st-esg-temperature" class="text_pole" type="number" min="0" max="2" step="0.1" /></label></div>
          ...
          <section class="st-esg-tab-panel" data-tab-panel="components">
            <div class="st-esg-card">
              ...
              <div class="st-esg-grid"><label>组件名<input id="st-esg-component-name" class="text_pole" type="text" placeholder="例如：人物状态栏" /></label><label>分类<select id="st-esg-component-scope" class="text_pole"><option>全局</option><option>预设</option><option>角色</option></select></label></div>
          ...
          <section class="st-esg-tab-panel" data-tab-panel="output">
            ...
            <div class="st-esg-card"><div class="st-esg-card-head"><div><div class="st-esg-card-title">输出清理</div><div class="st-esg-card-desc">每行一个标签或包裹符，用于清理模型多余输出。</div></div></div><textarea id="st-esg-cleanup-tags" class="text_pole textarea_compact st-esg-textarea" rows="5" placeholder="例如：&#10;<status>&#10;</status>"></textarea></div>
...
function mountUi() {
  if (!targetDoc.body) {
    targetWindow.setTimeout(mountUi, 500);
    return;
  }
...
function init() {
  ...
  console.log(`[${EXTENSION_ID}] 已加载，UI 挂载文档：${targetWindow === window ? 'current' : 'parent'}`);
}
